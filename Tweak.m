...
static vm_prot_t get_protection(void *sectionStart) {
  ...
  if (vm_region_64(task, &address, &size, VM_REGION_BASIC_INFO_64, (vm_region_info_t)&info, &info_count, &object) == KERN_SUCCESS) return info.protection;
#else
  vm_region_basic_info_data_t info;
  mach_msg_type_number_t info_count = VM_REGION_BASIC_INFO_COUNT;
  if (vm_region(task, &address, &size, VM_REGION_BASIC_INFO, (vm_region_info_t)&info, &info_count, &object) == KERN_SUCCESS) return info.protection;
#endif
  return VM_PROT_READ;
}
...
static void perform_rebinding_with_section(struct rebindings_entry *rebindings, section_t *section, intptr_t slide, nlist_t *symtab, char *strtab, uint32_t *indirect_symtab) {
  ...
  if (isDataConst) {
    protection = get_protection(rebindings);
